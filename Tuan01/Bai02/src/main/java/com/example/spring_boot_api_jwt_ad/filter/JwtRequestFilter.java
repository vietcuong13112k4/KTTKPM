...
public class JwtRequestFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        // Kiểm tra: user hợp lệ, token tồn tại, token chưa hết hạn, token chưa bị revoke, và là ACCESS token
        if (null != user && null != token 
                && token.getTokenExpDate() != null
                && token.getTokenExpDate().after(new Date())
                && (token.getIsRevoked() == null || !token.getIsRevoked())
